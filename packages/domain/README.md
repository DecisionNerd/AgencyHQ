...
- **`acceptance.ts`** — `evaluateAcceptance` collects every acceptance failure before returning. Reason codes cover proposal rejection, uncited or unsatisfied criteria, missing or mismatched evidence refs, failing verification results, missing or version-mismatched reviews, blocking findings, weak review profiles, non-distinct reviewers, and missing or mismatched human approvals. The `WorkerReport.checksRun` field is absent from the input by construction (R-014).
- **`integrity.ts`** — `detectVerifierTampering` produces a blocking `FindingLike` for each changed path that matches a protected path pattern (package manifests, lock files, tsconfig, biome, CI workflows, test configs). Any change to an approved verifier or its configuration is Review-blocking until the profile is re-versioned (TESTING.md §67-73).
