...
| `leases` | Time-bounded credential grants issued to worker containers. Primary key `id`; unique on `(attempt_id, generation, purpose, nonce_hash)`. Columns: `purpose` (enum: `provider`, `git-read`, `integrate`, `upload`), `run_id`, `attempt_id`, `generation`, `nonce_hash` (SHA-256 hex of the worker nonce — raw nonce never stored), `expires_at`, `revoked_at` (nullable), `used_at` (nullable). Material is assembled at request time from provider state or `project_credentials`; no credential bytes are stored in this table. |
| `attempt_artifacts` | Coordinator-persisted artifact bundles from worker containers. Unique on `(attempt_id, generation, kind, commit_id)`. Columns: `commit_id`, `diff_digest`, `changed_paths` (jsonb), `quarantine_patch` (nullable), `bundle_sha256`, `bundle_bytes`, `verified` (bool, default false), `received_at`. |
| `attempt_stop_evidence` | Structured stop evidence per `(attempt_id, generation)`, unique. Columns: `steps` (jsonb ordered array of `{ at, step, detail? }`), `received_at`. Upserted on conflict — later submissions overwrite steps. |
| `project_credentials` | AES-256-GCM encrypted project-level credentials keyed by `(project_id, purpose)` where purpose is `git-read` or `integrate`. Columns: `ciphertext`, `iv`, `tag` (all bytea), `key_version` (int), `created_at`, `updated_at`. Encrypted with the 64-hex key from `AGENCYHQ_SECRETS_KEY`. |
