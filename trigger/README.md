...
Provider credentials (API keys for model providers) are delivered by a coordinator `provider` lease at task start (P17.1/P18.3). The lease broker reads `auth.json` from `AGENCYHQ_OPENCODE_DATA_DIR` and serves it to the container via `prepareRuntime` in `trigger/src/lib/runtime.ts`. Credentials are never baked into the image. The full lease and per-run HOME path are described in `docs/engineering/EXECUTION_MODEL.md` (Container-profile execution path). Not yet exercised live; L2 trial pending.
...
- `runtime.ts`: `prepareRuntime({ runId, attemptId, generation, nonce, broker, env })` prepares the per-run execution environment. On the `container` profile: creates the per-run isolated HOME (via `runtime-home.ts`), requests a `provider` lease from the coordinator, writes `auth.json` at `0600`, requests an `upload` lease, and returns a `cleanup` function that deletes the HOME tree. On the `host` profile: returns the process HOME unchanged and a no-op cleanup. Returns `{ ok: false, failureKind }` on provider lease refusal (`provider_login_required`, `provider_expired`, `provider_unavailable`); never throws. No secret values appear in logs or metadata.
- `source.ts`: `materializeSource({ source, dir, broker, token })` downloads a git bundle from `GET /internal/source/:projectId?rev=<sha>` using the upload lease token, writes it to a temp file, `git clone`s it, checks out the requested revision, verifies `HEAD == source.revision`, and deletes the temp file. Returns `{ ok: false, failureKind }` on SHA mismatch (`revision_mismatch`), clone failure, or download failure. Never leaves a partial clone on disk.
- `artifact-upload.ts`: `exportAttemptBundle({ repoPath, commitId, baseRevision })` creates a thin git bundle of `commitId` against `baseRevision`, computes SHA-256 and byte count, and returns the bundle path (caller must delete). `uploadAttemptArtifact(args)` calls `exportAttemptBundle` then `POST /internal/attempts/:id/artifacts` (or `/checkpoints`) with the upload token and `ArtifactUploadMeta`. Never throws; returns `{ uploadStatus: "uploaded" | "failed" | "skipped", artifactRef? }`.
- `broker.ts`: `Broker` interface and `HttpBroker` class. Provides `requestLease(request)`, `downloadSourceBundle({ projectId, rev, token })`, `uploadArtifact(...)`, `uploadCheckpoint(...)`, `uploadStopEvidence(...)`. The base URL is read from `AGENCYHQ_COORDINATOR_INTERNAL_URL`. Only idempotent GETs (`downloadSourceBundle`) are retried (up to 3 times); POSTs are never retried. All requests carry bounded timeouts (30 s default; 60 s for bundle download). Secret values are never logged.
- `evidence.ts`: `collectStopEvidence({ stopNdjsonPath })` reads `stop.ndjson` from the run directory and converts each JSON-line entry into a `StopEvidenceUpload` step; lines with unknown step names are silently dropped. `uploadStopEvidence({ attemptId, generation, broker, token, stopNdjsonPath })` calls `collectStopEvidence` then `POST /internal/attempts/:id/stop-evidence`. Never throws; returns a status string.
