...
async function runIntegrateMergeV2(
  ...
  const payloadWithNonce = { ...payload, leaseNonce: nonce };
  const result = await runIntegrateMergeV2WithBroker(payloadWithNonce, runId, broker, runRoot);
...
// ---------------------------------------------------------------------------
// runIntegrateMergeV2WithBroker: the testable core of the v2 path.
//
// Accepts an injected broker so tests can run with FakeBroker. Contains all
// v2 logic: lease request, askpass write + cleanup, source materialization,
// and the push. No Trigger SDK metadata calls so it can run in tests.
//
// SECURITY: askpassToken never appears in logs, metadata, errors, or call
// records. The askpass script is written mode 0700 and deleted in finally.
// ---------------------------------------------------------------------------

export async function runIntegrateMergeV2WithBroker(
  payload: IntegrateMergePayloadV2 & { leaseNonce: string },
  runId: string,
  broker: import("../lib/broker.ts").Broker,
  runRoot: string,
): Promise<IntegrateMergeOutput> {
  ...
  if (!leaseResult.ok) {
    throw new Error(`integrate.merge: integrate lease refused: ${leaseResult.refusal.reason}`);
  }

  const grant = leaseResult.grant;
  if (grant.material.purpose !== "integrate") {
    throw new Error("integrate.merge: lease material purpose mismatch");
  }

  // askpassToken is a secret — never log or record it.
  const { askpassToken } = grant.material;

  // Source download authenticates with a download token: request a review-purpose
  // lease (token only, no material) with the same dispatch nonce (E7/E10).
  const downloadLease = await broker.requestLease({
    runId,
    attemptId: payload.attemptId,
    generation: payload.generation,
    purpose: "review",
  ...
  if (!downloadLease.ok) {
    throw new Error(`integrate.merge: download lease refused: ${downloadLease.refusal.reason}`);
  }
  if (downloadLease.grant.material.purpose !== "review") {
    throw new Error("integrate.merge: download lease material purpose mismatch");
  }
  const downloadToken = downloadLease.grant.material.token;
  ...
  try {
    ...
    if (!sourceResult.ok) {
      throw new Error(`integrate.merge source materialization failed: ${sourceResult.failureKind}`);
    }
    ...
    return await runIntegrateMerge(v1Payload, deps, runDir);
  } finally {
