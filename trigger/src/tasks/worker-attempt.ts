...
async function runV2(payload: WorkerAttemptPayloadV2, params: any): Promise<WorkerAttemptOutput> {
  ...
  // D1 / W-6: read nonce from the payload (generated by coordinator at dispatch),
  // not from an env var that nothing sets.
  const nonce = payload.leaseNonce ?? requireEnv("AGENCYHQ_LEASE_NONCE");
