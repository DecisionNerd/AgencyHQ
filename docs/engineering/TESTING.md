...
| R-017 | Given an unrelated Finding, when the Lead classifies it, then a backlog WorkItem is linked and the contract is unchanged. | Domain test; `apps/coordinator/test/integration/flow.remediate.test.ts`. |
| R-018 | Given a superseded contract, when a replacement Attempt is created, then the old Attempt's Review and VerificationResults do not transfer. | Version tests; `apps/coordinator/test/integration/flow.manifest.test.ts` (manifest mechanics: entry order, per-entry contract versions, base refresh from ledger); `apps/coordinator/test/integration/flow.manifest-next.test.ts` (next entry planned after integration). |
