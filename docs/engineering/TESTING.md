...
| --- | --- | --- |
| C1 — fresh install (R-012, R-021) | Given a fresh checkout and empty task-specific volumes on each declared supported Docker platform, when `docker compose up -d` runs, then services, migrations, internal auth, Trigger project and image registration complete without host Node/pnpm/OpenCode, SQL, token copying, or forwarding workarounds. | Automated real Compose smoke; pinned versions/digests, readiness and redacted bootstrap logs. L1 partial evidence (2026-09-09, linux/arm64 Docker Desktop, one host): [trials/2026-09-compose.md](trials/2026-09-compose.md#l1--packaging-bootstrap-and-task-image-2026-09-09) — showed fresh install completing (pinned digests, readiness response, redacted bootstrap logs, runtime probe and fixture checks passing in containers); other platforms, provider login, and automated Compose smoke not yet covered. |
| C2 — restart and failed bootstrap (R-021) | Given durable data and valid credentials, when bootstrap is interrupted/retried or containers are recreated/down/up, then existing resources and credentials are reused, data remains intact, and failure has an actionable status. | Integration tests with injected interruption plus a real restart trial; no volumes from unrelated installations are altered. L1 partial evidence (2026-09-09, linux/arm64 Docker Desktop, one host): [trials/2026-09-compose.md](trials/2026-09-compose.md#l1--packaging-bootstrap-and-task-image-2026-09-09) — showed restart with volumes untouched (bootstrap exited 0 within 23 s, "bootstrap already complete", no new deployment), and interrupted bootstrap resumed without duplicate org/project/token, with actionable readiness states; integration tests with injected interruption (`apps/bootstrap/test/interrupt.test.ts`, `apps/bootstrap/test/redeploy.test.ts`, `apps/bootstrap/test/deploy-output.test.ts`) ran in the unit gate at f5b7d05 (bootstrap suite 108 tests). |
| C3 — provider lifecycle (R-022) | Given no provider login, when the operator uses OpenCode in the setup container and recreates it, then new task containers reuse that login; expiry, concurrent refresh, logout, and unavailable providers produce explicit readiness states without exposing secrets. | Unit tests by path (declared from code; live evidence pending L2 trial): `apps/coordinator/src/provider/state.ts` — `readProviderState` produces `login_required`/`expired`/`ready`/`unavailable`; `apps/coordinator/src/readiness/provider.ts` — `buildProviderReadiness` maps provider status to `worker` readiness; `apps/coordinator/src/internal/leases.ts` — `issueLeaseBroker` gates on provider state; `apps/coordinator/test/integration/lease.broker.test.ts`. Provider auth.json written at 0600 and cleaned up: `trigger/src/lib/runtime.ts`. Redaction: `packages/contracts/src/lease.ts` (`redactLeaseGrant`). Pending: manual provider authorization for every claimed API-key/interactive flow, then actual task-container calls; live evidence pending L2 trial. |
| C4 — first real repair (R-023, R-025) | Given a clean install, when the operator links a disposable fixture repository, sets authority, and submits intent through the shipped setup flow, then real Lead/worker/verify/review/accept tasks complete at the exact artifact boundary without seed scripts, with later stages on different disposable containers. | Unit tests by path (declared from code; live evidence pending L2 trial): source materialization `trigger/src/lib/source.ts` (`trigger/test/source.test.ts`); artifact upload `trigger/src/lib/artifact-upload.ts`; broker client `trigger/src/lib/broker.ts`; admission `packages/domain/src/admission/artifact.ts` (`packages/domain/test/admission.test.ts`); coordinator artifact routes `apps/coordinator/src/internal/artifacts-router.ts` (`apps/coordinator/test/integration/artifacts.test.ts`); project credentials `packages/db/src/repos/project-credentials.ts` (`packages/db/test/integration/container-runtime.test.ts`). Pending: browser journey plus real Trigger/OpenCode run IDs, artifact SHAs/digests; live evidence pending L2 trial. |
| C5 — artifact and effect integrity (R-003, R-006, R-015, R-023) | Given a completed worker container has been deleted, when verification/review materialize its artifact and multi-repo manifest, then exact source is available; tampered/stale/cross-project objects are rejected, and only approved integration can advance the expected shared ref. | Unit/integration tests by path (declared from code; live evidence pending L2 trial): admission gate with 12 ordered checks `packages/domain/src/admission/artifact.ts` (`packages/domain/test/admission.test.ts`); coordinator artifact route cross-project lease rejection and stale-generation: `apps/coordinator/test/integration/artifacts.test.ts`; bundle import and SHA verify `apps/coordinator/src/git/bundle.ts` (`apps/coordinator/src/git/mirror.ts`); worker push blocked by scrubbed env `trigger/src/lib/env.ts` (`trigger/test/push-boundary.test.ts`). Pending: actual container deletion/materialization trial, denied worker push, successful authorized CAS integration and base-moved/unknown-effect cases; live evidence pending L2 trial. |
