...
| --- | --- | --- |
| R-001, R-004 | Given a `completed` Trigger run, when acceptance evidence is absent, then the step is not accepted. | Domain test. `reject` command (`apps/coordinator/src/commands/reject.ts`): transitions work item to `halted`; `invalidate_acceptance` (`apps/coordinator/src/commands/invalidate-acceptance.ts`): inserts a new `invalidate` decision without modifying historical rows, transitions to `reopened`. |
...
| R-019 | Given an operator returns, when the workspace opens, then changed outcomes, pending decisions, stale observations, and stop status are visible without logs. | `apps/coordinator/test/integration/return-view.test.ts`, `apps/web/test/view-helpers.test.ts`; browser tests: `apps/web/e2e/return-view.spec.ts` (return view renders four sections including pending decisions and continuing runs; Acknowledge button present); `apps/web/e2e/approve.spec.ts` (approve from decisions page removes entry; work item page confirm dialog); `apps/web/e2e/reject.spec.ts` (reject from decisions page decreases count; reject from work item page halts item); `apps/web/e2e/stop.spec.ts` (stop shows confirm dialog, closes on confirm, no error). Slice 5 execution trial (2026-09-08): live approve-via-UI on real stack; full record: [trials/2026-09-slice5.md](trials/2026-09-slice5.md). |
...
**Slice 5 browser layer (G10, 2026-09-08):** `pnpm test:browser` runs Playwright Chromium headless against a fake-runtime coordinator (`RUNTIME=fake`) with a seeded ledger (via `apps/coordinator/scripts/seed-control-plane.ts`). 16 journeys across `apps/web/e2e/*.spec.ts`: return after interruption (`return-view.spec.ts` — 3 tests), five work-item states with source and timestamp (`work-item.spec.ts` — 5 tests), approve pending-human from decisions page (`approve.spec.ts` — 2 tests), reject to halted (`reject.spec.ts` — 2 tests), authority edit — invalid schema error and valid save (`authority.spec.ts` — 2 tests), stop shows stopping/stopped (`stop.spec.ts` — 2 tests). CI job `browser` runs on ubuntu-latest with a Postgres 17.6 service (`.github/workflows/check.yml`). Gate G10 requires all 16 to pass before the slice PR merges. Slice 5 execution trial (approve via UI, real stack, 2026-09-08): [trials/2026-09-slice5.md](trials/2026-09-slice5.md).
