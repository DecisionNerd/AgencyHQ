...
**Slice 5 browser layer (G10, 2026-09-08):** `pnpm test:browser` runs Playwright Chromium headless against a fake-runtime coordinator (`RUNTIME=fake`) with a seeded ledger (via `apps/coordinator/scripts/seed-control-plane.ts`). 17 journeys across `apps/web/e2e/*.spec.ts`: return after interruption (`return-view.spec.ts` — 4 tests, including "ledger" source/timestamp check on completed item state cards), six work-item states (`work-item.spec.ts` — 6 tests), approve pending-human from decisions page and work-item page (`approve.spec.ts` — 2 tests), reject to halted (`reject.spec.ts` — 2 tests), authority edit — invalid schema error and valid save (`authority.spec.ts` — 2 tests), stop to "stopping" (`stop.spec.ts` — 1 test; the journey proves the deterministic "stopping" state only — the fake runtime does not produce adapter evidence for the full "stopped" cycle). CI job `browser` runs on ubuntu-latest with a Postgres 17.6 service (`.github/workflows/check.yml`). Browser suite was red at `be841ff` (journeys shared seeded items while a second worker consumed them), fixed at `04c3893` (per-journey seeded items, one worker); CI green at `04c3893` per PR #12 checks. Slice 5 execution trial (approve via UI, real stack, 2026-09-08): [trials/2026-09-slice5.md](trials/2026-09-slice5.md).
