...
| `integrate.merge` | After a recorded acceptance: dispatched by the coordinator on the `integrate` queue (`concurrencyLimit: 1`; callers supply a per-repository `concurrencyKey`). Steps: (1) `git fetch` the target ref to observe the current remote revision; (2) `git merge-base --is-ancestor` to confirm the attempt commit descends from the expected base; (3) `git worktree add` at the current remote tip; (4) `git merge` the attempt commit; (5) `git push --force-with-lease` with the expected base as the lease. Rejection classified as `lease_broken`, `auth`, `network`, or `other` from stderr. Outcomes: `integrated`, `already_integrated` (idempotent replay), `base_moved` (ref moved between fetch and push), `conflict` (merge conflict), `push_rejected` (push failed after merge). Only `integrate.merge` may push to any remote; no worker task holds push credentials. This boundary is enforced by the scrubbed worker environment (ADR-0007) and confirmed by `trigger/test/push-boundary.test.ts`. | `IntegrateMergeOutput` with outcome, resulting revision, and classified evidence. |
