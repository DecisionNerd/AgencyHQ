...
**Campaign-aware dispatch ordering and batch scheduler (Slice 6).** The coordinator's polling loop calls `selectDispatch` on every pass. `selectDispatch` (domain `packages/domain/src/dispatch.ts`) returns the highest-ranked eligible work items, up to `AGENCYHQ_WORKER_SLOTS`, and a skip reason for every other item. Active attempts (dispatched, running, or stopping) are deducted from the slot count before the pass; only attempts with an in-flight `worker.attempt` intent, or in `stopping` status, count — attempts left `dispatched` after a `verify.run`/`lead.review`/`lead.accept` dispatch do not count, and attempts belonging to halted or completed items do not count (`listActiveAttemptsForScheduling`, ecc3cd6). Provider capacity gating applies before the slot gate: if a work item's `provider`/`model` pair has a `ProviderCapacity` observation with `effective = "down"` the item is skipped `provider_down`; `limited` or `unknown` cap the provider's concurrent dispatch to 1 (`provider_limited` / `provider_unknown`). Observations past their `validUntil` are treated as `unknown` (conservative stale handling). Campaign rank and main effort surface in the overview sort order and in `main_effort_work_item_id`. Skip reasons — `repository_busy`, `no_slot`, `provider_down`, `provider_limited`, `provider_unknown`, `already_active`, `integration_pending` — are recorded and exposed in the overview via `skipReason`. Live verification of items T0–T3: T2 PASS (release point = worker-run completion; 20:52:10Z), T3 PASS (slot enforcement holds across passes; ecc3cd6), T4 PASS (operator capacity rows; 21:25–21:30Z) — see [trials/2026-09-slice6.md](trials/2026-09-slice6.md).
