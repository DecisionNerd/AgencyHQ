...
|----------|---------|---------|
| `AGENCYHQ_WORKER_SLOTS` | 1 | Maximum concurrent worker attempts across the shared ledger. On admission the worker intent is recorded `queued` and the flow runs the scheduling pass (`scheduleQueuedIntents`) immediately (`apps/coordinator/src/main.ts`), applying every gate at admission time. The polling loop also calls `selectDispatch` on every pass, deducting active attempts (`listActiveAttemptsForScheduling`: attempts in `stopping` status or with an in-flight `worker.attempt` intent, whose work item is not halted/completed/done) before selecting new ones. The provider gate is skipped entirely when the `provider_capacity` table is empty. The overview uses `listActiveAttemptCountsPerProject` with the same rule. Confirmed live 2026-09-08 (T3 PASS at ecc3cd6; S6R rework; see [docs/engineering/trials/2026-09-slice6.md](../../docs/engineering/trials/2026-09-slice6.md)). |
