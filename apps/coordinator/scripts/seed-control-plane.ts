 ...
 *
 * Runs migrations first.  Idempotency: if a project with clone_path
 * "/browser-test-control-plane" already exists, outputs its IDs and exits.
 *
...
const CLONE_PATH = "/browser-test-control-plane-v2";
...
try {
  ...
  if (existingRows.length > 0 && existingRows[0]) {
    ...
    const expectedTags = ["wiApprove", "wiReject", "wiCompleted", "wiAdmitted", "wiBlocked", "wiMerge"];
  ...
  await insertStepContract(client, {
    ...
    inputs: { defect: null },
    criteria: [
      { id: "c-blocked", text: "No deprecated API calls", source: "operator" },
    ],
    criteria_digest: CRITERIA_DIGEST,
    profile_id: "default",
    profile_digest: PROFILE_DIGEST,
