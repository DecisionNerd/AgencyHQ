...
- `api.ts` — types (`ReturnView`, `Item`, `State`, `Stop`, `PendingDecision`, etc.), token store (`getToken`, `setToken`, `clearToken` backed by `localStorage["agencyhq.apiToken"]`), and fetch helpers (`fetchReturnView(since)`, `fetchWorkItem(id)`, `fetchRealtimeToken(id)`, `postCommand(cmd)`). All fetch calls go to relative `/api/*` paths and include `Authorization: Bearer <token>` when a token is stored; a 401 response clears the token and throws `UnauthorizedError`.
- `App.tsx` — the main page. Reads `agencyhq.lastAckAt` from `localStorage` as the `since` parameter; displays sections in order: **Changed since your last visit**, **Decisions pending**, **Stops**, **Continuing**. An **Acknowledge visit** button posts `ack_visit` and updates `lastAckAt`. Each work item shows four state cards (contract, execution, verification, acceptance) plus a fifth **Integration** card when the item carries a non-null `integration` field (rendered with text and icon distinctions: ⏳ Pending integration, ✓ Integrated, ✗ Integration failed; outcome word, target ref, resulting revision as a 7-char short sha with full sha in a `title` attribute, timestamp, source "ledger"; a "Manifest n/m" line when `manifest` is non-null). The `mainEffort` item is highlighted and floated to the top of its section. When the API returns 401, a minimal token entry form is shown (label "API token", text input, submit); on submit the token is stored and the request is retried once.
